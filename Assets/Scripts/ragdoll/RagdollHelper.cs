...
public class RagdollHelper : MonoBehaviour {
    ...
    void Death()
    {
        for (var i = CameraFollow.instance.rivalsPos.Count - 1; i > -1; i--)
        {
            if (CameraFollow.instance.rivalsPos[i] == transform)
                CameraFollow.instance.rivalsPos.RemoveAt(i);
        }
        Debug.Log("done");
		...
		set{
            ...
            if (value==true){
                ...
                if (state==RagdollState.animated) {
				    ...
				    StopAllCoroutines();
                    StartCoroutine(StandUpBoy());
                } 
			}
			else {

                if (player.hites >= player.lives)
                {
    ...
    private Component[] componentsrb;
    ...
    void setKinematic(bool newValue)
	{
	    ...
	    componentsrb=GetComponentsInChildren(typeof(Rigidbody));

		//For each of the components in the array, treat the component as a Rigidbody and set its isKinematic property
		foreach (Component c in componentsrb)
		{
            (c as Rigidbody).velocity = Vector3.zero;
            (c as Rigidbody).angularVelocity = Vector3.zero;
	...
	void LateUpdate()
	{
		...
		if (state==RagdollState.blendToAnim)
		{
			if (Time.time<=ragdollingEndTime+mecanimToGetUpTransitionTime)
			{
              ...
              //  Debug.Log(ragdolledHipPosition);
                Vector3 animatedToRagdolled=ragdolledHipPosition-anim.GetBoneTransform(HumanBodyBones.Neck).position;
                ...
                transform.position = new Vector3(newRootPosition.x, y, newRootPosition.z);
